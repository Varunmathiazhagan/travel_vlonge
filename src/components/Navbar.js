...
import { MdTravelExplore, MdDirectionsBoat, MdBackpack, MdCardTravel } from "react-icons/md";
...
const Navbar = memo(({ cart, theme = 'light', isAuthenticated, setIsAuthenticated }) => {
  ...
  // Memoize theme-based styles with enhanced colors
  const themeStyles = useMemo(() => ({
    header: theme === 'light' 
      ? 'bg-gradient-to-r from-white/90 via-blue-50/90 to-indigo-50/90 backdrop-blur-lg'
      : 'bg-gradient-to-r from-gray-900/90 via-blue-900/90 to-indigo-900/90 backdrop-blur-lg text-white',
    nav: theme === 'light'
      ? 'hover:bg-blue-50/80 hover:text-blue-600'
      : 'hover:bg-indigo-700/80 hover:text-blue-300',
    activeLink: theme === 'light'
      ? 'bg-gradient-to-r from-blue-500 via-blue-600 to-indigo-500 text-white shadow-lg shadow-blue-500/20'
    ...
    languageMenu: theme === 'light'
      ? 'bg-white/95 backdrop-blur-md border border-blue-100 shadow-xl shadow-blue-200/50'
      : 'bg-gray-800/95 backdrop-blur-md border border-indigo-900 shadow-xl shadow-indigo-900/30 text-white',
    mobileMenu: theme === 'light'
      ? 'bg-white/95 backdrop-blur-lg border-t border-blue-100'
      : 'bg-gray-900/95 backdrop-blur-lg border-t border-indigo-800 text-white'
  }), [theme]);
  ...
  return (
    <motion.header
      className={`fixed top-0 w-full z-[100] transition-all duration-300 
        ${isScrolled 
          ? `h-14 sm:h-16 md:h-[4.5rem] ${themeStyles.header} shadow-xl ring-1 ring-blue-200/50 transform-gpu` 
    ...
    >
      <div className="container mx-auto h-full px-3 sm:px-4 flex justify-between items-center">
        <Link 
          ...
          aria-label={t("VoyageVista Home", "navbar")}
        >
          <motion.div
            whileHover={{ rotate: [0, -10, 10, -10, 0], transition: { duration: 0.5 } }}
          >
            <FaGlobeAmericas className="text-xl sm:text-2xl md:text-3xl text-blue-600 drop-shadow-md" />
          </motion.div>
          <span className="text-lg sm:text-xl md:text-2xl font-bold tracking-tight bg-clip-text text-transparent bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-600">
            {t("VoyageVista", "footer")}
          </span>
        </Link>
        ...
        <div className="flex items-center space-x-1 sm:space-x-2 md:space-x-4">
          ...
          <div className="relative">
            <button
              className={`p-1.5 sm:p-2 rounded-md ${theme === 'light' ? 'hover:bg-blue-50/80' : 'hover:bg-indigo-700/80'} transition-all duration-200 flex items-center`}
            ...
            {isAuthenticated ? (
              <>
                <motion.button
                  ...
                  className={`relative p-1.5 sm:p-2 rounded-md ${theme === 'light' ? 'hover:bg-blue-50/80' : 'hover:bg-indigo-700/80'} transition-all duration-200 group flex items-center`}
                  aria-label={t("User menu", "navbar")}
                >
                  <FaUser className="text-lg sm:text-xl group-hover:text-indigo-600 transition-colors" />
              ...
              <Link
                to="/login"
                className={`relative p-1.5 sm:p-2 rounded-md ${theme === 'light' ? 'hover:bg-blue-50/80' : 'hover:bg-indigo-700/80'} transition-all duration-200 group`}
          ...
          <Link
            to="/saved-trips"
            className={`relative p-1.5 sm:p-2 rounded-md ${theme === 'light' ? 'hover:bg-blue-50/80' : 'hover:bg-indigo-700/80'} transition-all duration-200 group hidden sm:flex`}
            aria-label={t("Saved Trips", "navbar")}
          >
            <motion.div
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
              transition={{ type: "spring", stiffness: 400, damping: 17 }}
            >
              <FaBookmark className="text-lg sm:text-xl group-hover:text-indigo-600 transition-colors" />
            </motion.div>
          </Link>
          ...
          <Link
            to="/cart"
            className={`relative p-1.5 sm:p-2 rounded-md ${theme === 'light' ? 'hover:bg-blue-50/80' : 'hover:bg-indigo-700/80'} transition-all duration-200 group`}
            aria-label={t("Travel Cart", "navbar") + ` ${cartItemCount}`}
          >
            <motion.div
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
              transition={{ type: "spring", stiffness: 400, damping: 17 }}
            >
              <MdCardTravel className="text-lg sm:text-xl group-hover:text-indigo-600 transition-colors" />
              <AnimatePresence>
                {cartItemCount > 0 && (
                  <motion.span
                    initial={{ scale: 0 }}
                    animate={{ scale: 1 }}
                    exit={{ scale: 0 }}
                    className="absolute -top-1.5 -right-1.5 sm:-top-2 sm:-right-2 bg-gradient-to-r from-indigo-500 to-blue-600 text-white text-xs 
                             rounded-full w-4 h-4 sm:w-5 sm:h-5 flex items-center justify-center 
                             shadow-md sm:shadow-lg shadow-indigo-500/30 text-[10px] sm:text-xs"
                  >
                    {cartItemCount}
                  </motion.span>
                )}
          ...
          <motion.button
            whileTap={{ scale: 0.95 }}
            className={`lg:hidden p-1.5 sm:p-2 rounded-md ${theme === 'light' ? 'hover:bg-blue-50/80' : 'hover:bg-indigo-700/80'} transition-colors duration-200`}
              ...
              {navLinks.map((link, index) => (
                ...
                >
                  <Link
                    ...
                    className={`flex items-center px-3 sm:px-4 py-2.5 sm:py-3 rounded-lg transition-all duration-200
                              ${location.pathname === link.to
                                ...
                                : `${theme === 'light' ? 'hover:bg-blue-50/80 hover:text-indigo-600' : 'hover:bg-indigo-800/80 hover:text-blue-300'}`}`}
                  ...
                  {index < navLinks.length - 1 && (
                    <div className={`border-b ${theme === 'light' ? 'border-blue-100' : 'border-indigo-800'} my-1 opacity-70`} />
                  )}
                </motion.div>
              ))}
                ...
                <>
                  ...
                  >
                    <Link
                      to="/profile"
                      onClick={() => setIsMobileMenuOpen(false)}
                      className={`flex w-full items-center px-3 sm:px-4 py-2.5 sm:py-3 rounded-lg ${theme === 'light' ? 'bg-indigo-50 text-indigo-700' : 'bg-indigo-800 text-blue-300'} transition-all duration-200`}
                    >
                  ...
                  >
                    <Link
                      to="/bookings"
                      onClick={() => setIsMobileMenuOpen(false)}
                      className={`flex w-full items-center px-3 sm:px-4 py-2.5 sm:py-3 rounded-lg ${theme === 'light' ? 'bg-blue-50 text-blue-700' : 'bg-blue-800 text-blue-300'} transition-all duration-200`}
                    >
                      <span className="text-lg sm:text-xl"><FaBookmark /></span>
                      <span className="ml-3 font-medium text-sm sm:text-base">{t("My Bookings", "navbar")}</span>
                    </Link>
                    <div className={`border-b ${theme === 'light' ? 'border-blue-100' : 'border-indigo-800'} my-1 opacity-70`} />
                  </motion.div>
                  ...
                  >
                    <button
                      ...
                      className={`flex w-full items-center px-3 sm:px-4 py-2.5 sm:py-3 rounded-lg bg-gradient-to-r from-red-500 to-pink-600 text-white transition-all duration-200 hover:from-red-600 hover:to-pink-700`}
                    >
                      <span className="text-lg sm:text-xl"><FaUser /></span>
